...
Usage:
    dml process <input> [<output>] [--format=<fmt> | \
                             --partial]
    dml process --help
...
Example:
    dml \
                             process input_file.dml output_file.html
    dml process in.dml \
                             out.tex --format=latex

Notes:
    Partial output let you get html \
                             or latex without header
    and without footer. It is useful if you \
                             want to insert
...
impl Command for ProcessCommand {
    fn run(&self, argv: &Vec<String>) {
        ...
        let output: Box<Write> = match args.arg_output {
            Some(path) => Box::new(File::create(path).expect("Couldn't load output file")),
            None => Box::new(stdout()),
        };
